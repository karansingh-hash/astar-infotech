...
const SV=[{i:'Globe',t:'Website Design',d:'Beautiful, modern designs that capture your brand identity.',c:'text-emerald-400',bg:'bg-emerald-500/10',img:'/service-web-design.png'},{i:'Code2',t:'Website Development',d:'Robust, scalable web apps built with the latest technologies.',c:'text-cyan-400',bg:'bg-cyan-500/10',img:'/service-web-development.png'},{i:'ShoppingCart',t:'E-Commerce Development',d:'Feature-rich online stores with secure payments.',c:'text-emerald-400',bg:'bg-emerald-500/10',img:'/service-ecommerce.png'},{i:'Smartphone',t:'Responsive Websites',d:'Websites that look stunning on every device.',c:'text-cyan-400',bg:'bg-cyan-500/10',img:'/service-responsive.png'},{i:'Settings',t:'Website Maintenance',d:'Ongoing support to keep your site running smoothly.',c:'text-emerald-400',bg:'bg-emerald-500/10',img:'/service-maintenance.png'},{i:'Search',t:'SEO Services',d:'Data-driven SEO strategies that boost visibility.',c:'text-cyan-400',bg:'bg-cyan-500/10',img:'/service-seo.png'},{i:'Smartphone',t:'App Development',d:'Native and cross-platform mobile apps that deliver seamless user experiences.',c:'text-cyan-400',bg:'bg-cyan-500/10',img:'/service-app-development.png'}]
...
if(sR.status==='fulfilled'&&sR.value.ok){const d=await sR.value.json();if(d.services?.length)setSvs(d.services.map((x:any)=>({i:x.icon||'Globe',t:x.title,d:x.description,c:x.color||'text-emerald-400',bg:x.bgColor||'bg-emerald-500/10',img:x.image||''})))}
...
{svs.map((svc,idx)=>{const Ic=IM[svc.i]||Globe;return<Card key={svc.t+idx} className={`group h-full ${tc} card-hover-lift relative overflow-hidden`}>{svc.img&&<div className="relative h-40 sm:h-48 overflow-hidden"><img src={svc.img} alt={svc.t} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"/><div className="absolute inset-0 bg-gradient-to-t from-background via-background/30 to-transparent"/><span className="service-number">{String(idx+1).padStart(2,'0')}</span></div>}<CardContent className="p-4 md:p-6 relative z-10">{!svc.img&&<span className="service-number">{String(idx+1).padStart(2,'0')}</span>}<div className={`icon-ring w-10 h-10 rounded-full ${svc.bg} flex items-center justify-center mb-3 group-hover:scale-110 transition-transform border border-neon/20`}><Ic className={`w-4 h-4 ${svc.c}`}/></div><h3 className={`text-lg font-semibold text-foreground mb-2 ${th}`}>{svc.t}</h3><p className="text-sm text-muted-foreground leading-relaxed">{svc.d}</p><a href="#contact" className="inline-flex items-center gap-1 mt-3 text-sm font-medium text-neon hover:text-neon/80 transition-colors min-h-[44px] group/l">Learn More<ArrowRight className="w-4 h-4 transition-transform group-hover/l:translate-x-1"/></a></CardContent></Card>})}
</div></div></section>
