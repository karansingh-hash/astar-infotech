...
{svs.map((svc,idx)=>{const Ic=IM[svc.i]||Globe;return<Card key={svc.t} className="group h-full service-card-blend card-hover-lift relative overflow-hidden"><span className="service-number">{String(idx+1).padStart(2,'0')}</span><CardContent className="p-4 md:p-8 relative z-10"><div className={`icon-ring w-12 h-12 rounded-full ${svc.bg} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform border border-neon/40 backdrop-blur-sm`}><Ic className={`w-5 h-5 ${svc.c} drop-shadow-[0_2px_4px_rgba(0,0,0,1)]`}/></div><h3 className="text-lg font-semibold text-white mb-2 drop-shadow-[0_2px_6px_rgba(0,0,0,1)] drop-shadow-[0_0_2px_rgba(0,0,0,1)]">{svc.t}</h3><p className="text-sm text-white/95 leading-relaxed drop-shadow-[0_2px_5px_rgba(0,0,0,1)] drop-shadow-[0_0_2px_rgba(0,0,0,1)]">{svc.d}</p><button onClick={()=>setSDlg({open:true,svc})} className="inline-flex items-center gap-1 mt-3 text-sm font-medium text-neon hover:text-white hover:bg-neon/20 px-3 py-1.5 rounded-lg transition-colors min-h-[44px] group/l backdrop-blur-sm border border-neon/40 drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">Learn More<ArrowRight className="w-4 h-4 transition-transform group-hover/l:translate-x-1"/></button></CardContent></Card>})}
</div></div></section>
